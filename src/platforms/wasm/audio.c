...
static bool unqueue_buffers(void)
{
	ALint val;
...
/* Never busy-wait: a spin here freezes the browser tab. */
...
size_t audio_write(const void *buf_, unsigned size) {
	...
	while (size)
	{
		...
		if (!get_buffer(&buffer)) {
			/* Drop this chunk rather than blocking the main thread. */
			al->tmpbuf_ptr = 0;
			break;
		}
...
void rb_audio_init(int rate) {
	...
	if (!al->handle) {
		fprintf(stderr, "OpenAL: no device (audio muted)\n");
		free(al);
		al = NULL;
		return;
	}
	...
	if (!al->ctx) {
		fprintf(stderr, "OpenAL: no context (audio muted)\n");
		alcCloseDevice(al->handle);
		free(al);
		al = NULL;
		return;
