...
The frontend is driven by an ini file instead of command-line flags so a core and a ROM can ship as one folder (desktop) or one NRO (Switch). Desktop uses glad and OpenAL; Switch uses Mesa EGL and libnx `audout`.

Usage: [doc/desktop.md](doc/desktop.md) (Linux, Windows, macOS) and [doc/switch.md](doc/switch.md) (Nintendo Switch).
...
Each command is `cmake --preset` inside the image (`linux`, `windows`, `macos-x86_64`, `macos-arm64`, `switch`). Desktop ports statically link GLFW and OpenAL-Soft. Switch uses libnx + Mesa EGL (no GLFW). Zips land in `dist/`.
...
`sdk` is an alias for `shell`. Then `cmake --preset switch` (or `linux` / `windows` / `macos-x86_64`).

Zed, VS Code, and Cursor can attach to the same images via [Dev Containers](https://containers.dev/) (`.devcontainer/`). The default is Linux; pick **Windows (MinGW)**, **macOS (osxcross)**, or **Switch (devkitA64)** in the config picker.
